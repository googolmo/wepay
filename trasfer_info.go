...
import (
	...
	"github.com/googolmo/wepay/util"
)
...
// GetInfo 转账信息
func (t TransferInfo) GetInfo(key string, x509KeyPair tls.Certificate) (res TransferInfoResponse, err error) {
	reqData, err := t.prepare(key)
	if err != nil {
		return
	}

	resData, err := util.TSLPostXML(paymentBaseURL+transferInfoAPI, reqData, x509KeyPair)
	if err != nil {
		return
	}

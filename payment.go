...
const (
	...
	paymentBaseURL = "https://api.mch.weixin.qq.com"
...
func (o Order) Unify(key string) (pres PaidResponse, err error) {
	...
	data, err := util.PostXML(paymentBaseURL+unifyAPI, reqData)
	if err != nil {
		return
	}
...
func (q Query) Query(key string) (resp OrderResponse, err error) {
	...
	q.Sign = sign
	data, err := util.PostXML(paymentBaseURL+queryAPI, q)
	if err != nil {
		return
	}
...
func GetPaidUnionID(accessToken, openID, transactionID string) (*GetPaidUnionIDResponse, error) {
	api := wxBaseURL + getPaidUnionIDAPI
